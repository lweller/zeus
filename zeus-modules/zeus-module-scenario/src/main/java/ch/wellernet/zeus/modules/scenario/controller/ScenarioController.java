...
public class ScenarioController implements ScenarioApiV1Controller {
  ...
  public ResponseEntity<Scenario> findById(
    ...
    return ResponseEntity.status(OK).body(scenarioRepository.findById(id).orElseThrow(NoSuchElementException::new));
  }
  ...
  private Scenario findScenario(final UUID id) throws OptimisticLockException {
    return scenarioRepository.findById(id).orElseThrow(NoSuchElementException::new);
  }
}
